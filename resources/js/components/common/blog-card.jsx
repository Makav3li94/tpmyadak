...
export default function BlogCard({blog,i}) {
    return (
        <article key={i} className="flex flex-col items-start justify-between card card-sm c-shadow p-4">
            <div className="article-date">
                <div className="date"><span className="article-date">
                    <b>{blog.fucking_pub_at[1]}</b>{blog.fucking_pub_at[0]}
                </span>
                </div>
            </div>
            <div className="relative w-full article-image">
                <img alt={blog.title} src={route('file.show', blog.img_cover)}
                     className="aspect-[16/9] w-full rounded bg-gray-100 object-cover sm:aspect-[2/1] lg:aspect-[3/2] group-hover:opacity-75"
                />
            ...
            <div className="max-w-xl">
                <div className="mt-8 flex items-center gap-x-4 text-xs">
                    ...
                    <Link href={route('home.getBlogs', {category: blog.category.id})}
