...
class ProductControllernew extends Controller
{
    ...
    public function edit(Product $product)
    {
        ...
        return inertia('admin/product/form', [
            'product' => $product->load(['category:id,parent_id,title,slug', 'brand:id,title,slug', 'supplier:id,title,slug', 'tax:id,title,value', 'specs', 'attributes', 'filters']),
            'prFiles' => $prFiles,
            'productCategories' => $categories,
            'attrGroups' => $attrGroups,
            'carModels' => $carModels,
    ...
    private function handleValidate(Request $request, $isUpdate = false, $id = null): array
    {
        ...
        $defaultValidationArray = [
            ...
            'excerpt' => 'required|string|max:125',
...
//            'sku' => 'required|numeric',
    ...
    /**
     * Optimized getLabels:
     * - Fetches all categories in a single query and builds a nested tree in-memory (O(n))
     * - Uses selective column selection and eager loading for car models
     * - Optionally caches the result to avoid repeated DB hits
     *
     * Returns array: [categories, attrGroups, carModels]
     */
    private function getLabels(): array
    {
        // Cache key and TTL (seconds). Adjust TTL as needed or remove caching if dynamic updates are frequent.
        $cacheKey = 'admin:product_labels';
        $ttl = 300; // 5 minutes

        return Cache::remember($cacheKey, $ttl, function () {
            // 1) Fetch all categories in single query (no N+1)
            $rawCategories = ProductCategory::select('id', 'parent_id', 'title', 'slug')
                ->orderBy('parent_id')
                ->orderBy('title')
                ->get()
                ->toArray();

            // Build map of nodes (O(n)). Use string keys for consistent JSON output.
            $nodes = [];
            ...
            // Attach nodes to parents to create tree (single pass, O(n)).
            $tree = [];
            foreach ($rawCategories as $cat) {
                $id = (string) $cat['id'];
            ...
            // Convert 'children' to 'options' recursively to match expected front-end structure
            $convertChildren = function (&$items) use (&$convertChildren) {
                foreach ($items as &$item) {
                    if (!empty($item['children'])) {
                        $item['options'] = $item['children'];
                        // recursively convert grandchildren
                        $convertChildren($item['options']);
                    } else {
                        $item['options'] = [];
                    }
                    // remove the temporary 'children' key
                    unset($item['children']);
                }
            };
            ...
            // 2) Attribute groups (simple selective query)
            $attrGroups = AttributeGroup::select('id', 'title')
                ->where('status', 1)
                ->get();

            // 3) Car brands with their car models (eager loaded, limited columns)
            $carModels = CarBrand::with(['carModels' => function ($q) {
                $q->select('id', 'car_brand_id', 'title');
            }])
                ->select('id', 'title')
                ->get()
                ->map(function ($item) {
                    return [
                        ...
                        'label' => $item['title'],
                        'options' => $item['carModels']->map(function ($cm) {
                            return [
                                'value' => strval($cm['id']),
                                'label' => $cm['title'],
                            ];
            ...
            return [$tree, $attrGroups, $carModels];
        });
    }
