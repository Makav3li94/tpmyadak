...
//        })->get());
    ...
    {
        list($categories, $attrGroups, $carModels) = $this->getLabels();
    ...
    public function store(Request $request)
    {
        ...
        if (count($filterArray))$product->carModels()->attach(array_mapper($filterArray));
    ...
    public function update(Request $request, Product $product): RedirectResponse
    {
        ...
        $filterArray = $request['car_model_id'];
        unset($validatedData['car_model_id']);
        if (count($filterArray))$product->carModels()->sync(array_mapper($filterArray));
...
//            'img_cover' => 'required|string',
        ...
        if (isset($validatedData['date_start'])) {
            if (!isNull($validatedData ['date_start'])) $validatedData ['date_start'] = Carbon::parse($validatedData ['date_start'])->toDateTimeString();
        }
        if (isset($validatedData['date_end'])) {
            if (!isNull($validatedData ['date_end'])) $validatedData ['date_end'] = Carbon::parse($validatedData ['date_end'])->toDateTimeString();
        }
        if (is_array($validatedData ['product_category_id'])) $validatedData ['product_category_id'] = $request->product_category_id['value'];
    ...
    private function handleProductGroupAttrs(Request $request, Product $product, $type = 'store'): void
    {
        ...
        if (isset($request->product_group_attrs)) {
            ...
            if ($type == 'update') ProductAttribute::where('product_id', $product->id)->delete();
    ...
    private function handleProductSpecs(Request $request, Product $product, $type = 'store'): void
    {
        ...
        if (isset($request->product_group_specs)) {
            ...
            if ($type == 'update') ProductSpecs::where('product_id', $product->id)->delete();
...
//            }
    ...
    private function getLabels(): array
    {
        ...
        return array($categories, $attrGroups, $carModels);
    }
